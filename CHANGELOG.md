...
## 未发布
- 新增 LlamaIndex RAG 行文参考能力：成稿阶段基于章节计划与角色贡献检索小说知识库片段，优先学习口语化动作/对白表达，降低 AI 味（仅学习表达，不照抄句子与情节）。
- 新增 `rag-index` CLI 命令：支持将用户提供的 `.txt` 小说递归切分后写入 Chroma 向量库（可 `--rebuild` 重建）。
- 新增智谱 Embedding 适配：接入 `langchain_community` 的 `ZhipuAIEmbeddings`，并增加 64 条上限分批机制，避免单次请求超限。
- 新增 RAG 配置项：`rag.*`（开关、分块、检索参数）与 `paths.rag_*`（语料目录、向量库目录、集合名）。
- 连续性上下文注入阶段调整为计划阶段：读取已生成章节时紧邻当前章节的前 3 章注入全文，其余章节注入剧情摘要以提升剧情连贯性。
